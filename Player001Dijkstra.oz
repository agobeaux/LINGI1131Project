...
in
   ...
   fun {BFS Map Start Target}
      fun {DoBFS Map Start Queue Seen Target}
         ...
         of nil then
            ...
            notarget
         [] node(x:X y:Y dist:D)|T then
            if X =< 1 orelse X >= Input.nbColumn orelse Y =< 1 orelse Y >= Input.nbRow orelse ({Not {IsGoodMap {Value Map X Y}}} andthen {Not Target == {Value Map X Y}}) then
   ...
   fun {DoAction Summary RetID RetAction}
      ...
      elseif Summary.bombs =< 0 then
         local DMap X Y Up Down Left Right Prio NewRec NewPos BFSPoint BFSBonus BFSBBox BFSPBox in
            if Summary.pos \= nil then 
               ...
               BFSBBox = {BFS Summary.map pt(x:X y:Y) 3}
               BFSPBox = {BFS Summary.map pt(x:X y:Y) 2}

               if BFSBonus == notarget then
                  if BFSPoint == notarget then
                     if BFSBBox == notarget then
                        if BFSPBox == notarget then
                           NewRec = {BestMove Prio {RandomNeighbour X Y}}
                        else
                           NewRec = {BestMove Prio {BFSNeighbours Summary.map X Y 2 BFSPBox}}
                        end % if
                     else
                         NewRec = {BestMove Prio {BFSNeighbours Summary.map X Y 3 BFSBBox}}
                     end % if
                  else
         ...
         if Summary.pos \= nil then 
            ...
            else Up Down Left Right Prio NewRec NewPos BFSPoint BFSBonus BFSBBox BFSPBox in
               Up = rec(x:X y:Y-1 val:{Nth {Nth DMap Y-1} X})
               Down = rec(x:X y:Y+1 val:{Nth {Nth DMap Y+1} X})
               Left = rec(x:X-1 y:Y val:{Nth {Nth DMap Y} X-1})
               Right = rec(x:X+1 y:Y val:{Nth {Nth DMap Y} X+1})
